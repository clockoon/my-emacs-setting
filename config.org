...
*** neotree
많은 모던 에디터, 예컨대 sublime text나 atom, vscode처럼 사이드바에 트리 기능을 지원하도록 하는 패키지입니다.

#+BEGIN_SRC emacs-lisp :tangle yes
(use-package neotree
  :ensure t
  :config
  ;;(setq neo-theme (if (display-graphic-p) 'icons 'arrow)) ;; 화살표 테마 설정
  (global-set-key [f9] 'neotree-toggle)) ;; 토글 단축키 설정
#+END_SRC
